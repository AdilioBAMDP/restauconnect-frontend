...
export default function MessagesPage({ navigateTo }: MessagesPageProps) {
  ...
  useEffect(() => {
    ...
    console.log('🔍 MessagesPage useEffect déclenché:', {
      hasLoadedFromUrl: hasLoadedFromUrl.current,
      isLoading,
    ...
    // ✅ FIX ULTIME: Si query params présents, traiter TOUJOURS (ignorer hasLoadedFromUrl)
    if (!isLoading && conversationId === 'start' && partnerId && partnerName) {
    ...
    } else if (!isLoading && conversationId && conversationId !== 'start') {
  ...
  }, [fetchConversationById, fetchConversations, conversations, createConversation, isLoading]); // Ajout de isLoading
  ...
  // NOUVEAU - Supprimer conversation avec confirmation
  const handleDeleteConversation = async (conversationId: string) => {
    const otherParticipant = activeConversation?.participants?.find(
      p => p.userId !== currentUserId
    );
    
    const partnerName = otherParticipant?.userName || 'ce partenaire';
    
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer cette conversation avec ${partnerName} ?\n\nCette action est irréversible.`)) {
      logger.info('MessagesPage: Suppression conversation...', conversationId);
      const success = await deleteConversation(conversationId);
      if (success) {
        logger.info('MessagesPage: Conversation supprimée avec succès');
        setSelectedConversationId(null);
        // Recharger les conversations
        await fetchConversations();
      } else {
        logger.error('MessagesPage: Échec suppression conversation');
      }
    }
  };
